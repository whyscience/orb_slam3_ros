...
mvuRight(static_cast<vector<float> >(NULL)), mvDepth(static_cast<vector<float> >(NULL)),*/
        ...
        mvInvLevelSigma2(0), mnMinX(0), mnMinY(0), mnMaxX(0), mnMaxY(0), mPrevKF(static_cast<KeyFrame *>(NULL)),
        mNextKF(static_cast<KeyFrame *>(NULL)), mbFirstConnection(true), mpParent(NULL), mbNotErase(false),
    ...
    void KeyFrame::SetBadFlag()
    {
...

        {
            ...
            while (!mspChildrens.empty())
            {
                ...
                for (set<KeyFrame *>::iterator sit = mspChildrens.begin(), send = mspChildrens.end(); sit != send;
                     sit++)
                {
                    KeyFrame *pKF = *sit;
                    if (pKF->isBad())
            ...
            // If a children has no covisibility links with any parent candidate, assign to the original parent of this
            // KF
