...
namespace ORB_SLAM3
{
    ...
    bool LoopClosing::NewDetectCommonRegions()
    {
        ...
        {
            mbLoopDetected =
                    DetectCommonRegionsFromBoW(vpLoopBowCand, mpLoopMatchedKF, mpLoopLastCurrentKF, mg2oLoopSlw,
        ...
        for (KeyFrame *pKFi: vpBowCand)
        {
            ...
            if (numBoWMatches >= nBoWMatches) // TODO pick a good threshold
            {
                ...
                while (!bConverge && !bNoMore)
                {
                    ...
                    // Verbose::PrintMess("BoW guess: Solver achieve " + to_string(nInliers) + " geometrical inliers
                    // among " + to_string(nBoWInliers) + " BoW matches", Verbose::VERBOSITY_DEBUG);
                }
                ...
                if (bConverge)
                {
                    ...
                    // Verbose::PrintMess("BoW guess: Convergende with " + to_string(nInliers) + " geometrical inliers
                    // among " + to_string(nBoWInliers) + " BoW matches", Verbose::VERBOSITY_DEBUG);
                    ...
                    // std::cout << "There are " << vpKeyFrames.size() <<" KFs which view all the mappoints" <<
                    // std::endl;
                    ...
                    // cout <<"BoW: " << numProjMatches << " matches between " << vpMapPoints.size() << " points with
                    // coarse Sim3" << endl;
    ...
    void LoopClosing::CorrectLoop()
    {
...

        {
            ...
            // Correct all MapPoints obsrved by current keyframe and neighbors, so that they align with the other side
            // of the loop
        ...
        double timeOptEss =
                std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndOpt - time_EndFusion)
                        .count();
    ...
    void LoopClosing::MergeLocal()
    {
        ...
        // Later, the elements of the current map will be transform to the new active map reference, in order to keep
        // real time tracking
        ...
        pNewParent = mpCurrentKF; // Old child, now it will be the parent of its own parent(we need eliminate this KF
        ...
        double timeOptEss = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndOptEss -
                                                                                                  time_EndWeldingBA)
                                    .count();
    ...
    void LoopClosing::MergeLocal2()
    {
        ...
        pNewParent = mpMergeMatchedKF; // Old child, now it will be the parent of its own parent(we need eliminate this
                                       // KF from children list in its old parent)
        ...
        vector<MapPoint *> vpCheckFuseMapPoint; // MapPoint vector from current map to allow to fuse duplicated points
                                                // with the old map (merge)
    ...
    void LoopClosing::RunGlobalBundleAdjustment(Map *pActiveMap, unsigned long nLoopKF)
    {
        ...
        double timeGBA =
                std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndGBA - time_StartFGBA)
                        .count();
        ...
        // We need to propagate the correction through the spanning tree
        {
            ...
            if (!mbStopGBA)
            {
                ...
                while (!lpKFtoCheck.empty())
                {
                    ...
                    connected KFs" << endl; cout << "--It has " << pKF->GetCovisiblesByWeight(80).size() << " connected
                    KF with 80 common matches or more" << endl; cout << "--It has " <<
                    pKF->GetCovisiblesByWeight(50).size() << " connected KF with 50 common matches or more" << endl;
                        cout << "--It has " << pKF->GetCovisiblesByWeight(20).size() << " connected KF with 20 common
                    matches or more" << endl;
                        ...
                        for(int i=0; i<vpMapPointsKF.size(); ++i)
                        {
                            ...
                            cv::putText(imLeft, strNumOBs, pKF->mvKeys[i].pt, CV_FONT_HERSHEY_DUPLEX, 1, cv::Scalar(255,
                    0, 0));
                        }
