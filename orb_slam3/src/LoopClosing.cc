...
namespace ORB_SLAM3
{
        ...
        for (KeyFrame *pKFi: vpBowCand)
        {
            ...
            if (numBoWMatches >= nBoWMatches) // TODO pick a good threshold
            {
                ...
                if (bConverge)
                {
                    ...
                    // 拿到solver 估计的 Scm初始值, 为后续的非线性优化做准备, 在这里 c 表示当前关键帧, m
                    // 表示回环/融合候选帧
                    ...
                    if (numProjMatches >= nProjMatches)
                    {
                        ...
                        if (numOptMatches >= nSim3Inliers)
                        {
                            ...
                            // 5 : 半径的增益系数(对比之前下降了)---> 更小的半径, 1.0 , hamming distance
                            // 的阀值增益系数---> 允许更小的距离
                            ...
                            if (numProjOptMatches >= nProjOptMatches)
                            {
                                ...
                                if (nBestMatchesReproj < numProjOptMatches)
                                {
                                    ...
                                    g2oBestScw = gScw; // 记录最优的位姿(这个位姿是由Tam推到出来的 : Taw = Tam *
                                                       // Tmw,这里a表示c)
    ...
    vnLoopKFs.push_back(numKFinLoop);*/
        ...
        // 对地图点操作
        {
            ...
            // of the loop 3.2：得到矫正的当前关键帧的共视关键帧位姿后，修正这些关键帧的地图点
        ...
        // 投影到当前关键帧组中，进行匹配，融合，新增或替换当前关键帧组中KF的地图点 因为
    ...
    void LoopClosing::MergeLocal()
    {
        ...
        // real time tracking 当前关键帧的共视关键帧和他们观测到的地图点会先被融合, 融合后的图会变成新的当前激活地图.
