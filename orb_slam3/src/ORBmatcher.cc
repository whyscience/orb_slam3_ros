        ...
        for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++)
        {
            ...
            if (pMP->mbTrackInView)
            {
                ...
                if (!vIndices.empty())
                {
                    ...
                    if (bestDist <= TH_HIGH)
                    {
                        ...
                        // 条件2：bestDist>mfNNratio*bestDist2 表示最佳和次佳距离不满足阈值比例。理论来说
                        // bestDist/bestDist2 越小越好
            ...
            if (F.Nleft != -1 && pMP->mbTrackInViewR)
            {
                ...
                if (nPredictedLevel != -1)
                {
                    ...
                    const vector<size_t> vIndices = F.GetFeaturesInArea(pMP->mTrackProjXR, pMP->mTrackProjYR,
                        ...
                        if (static_cast<float>(bestDist1) < mfNNratio * static_cast<float>(bestDist2))
                        {
                            ...
                            if (mbCheckOrientation)
                            {
                                ...
                                if (rot < 0.0)
                                    rot += 360.0f;
                                int bin = round(rot *
                                                factor); // 将rot分配到bin组, 四舍五入, 其实就是离散到对应的直方图组中
                                if (bin == HISTO_LENGTH)
                                    bin = 0;
