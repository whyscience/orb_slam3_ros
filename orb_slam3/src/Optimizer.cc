...
namespace ORB_SLAM3
{
                                    ...
                                    int &num_MPs, int &num_edges, bool bLarge, bool bRecInit)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            ...
            if (pKFi->bImu && pKFi->mPrevKF->bImu && pKFi->mpImuPreintegrated)
            {
                ...
                // 陀螺仪偏置边连接了上一个关键帧的陀螺仪偏置节点VG1和当前迭代关键帧陀螺仪偏置节点VG2
                // 这也是ORB-SLAM2中没有的。
    ...
    void Optimizer::OptimizeEssentialGraph4DoF(
        ...
        Edge4DoF *e_loop;
        for (map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend = LoopConnections.
                             end(); mit != mend; mit++) {
            // 3.1 取出帧与帧们
            KeyFrame *pKF = mit->first;
            const long unsigned int nIDi = pKF->mnId;
