...
namespace ORB_SLAM3
{
     ...
     */
    Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight,
                                           const double &timestamp, string filename)
    {
        ...
        if (mSensor == System::STEREO && !mpCamera2)
            ...
            mCurrentFrame =
                    Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK,
    ...
    void Tracking::PreintegrateIMU()
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            if ((i == 0) && (i < (n - 1)))
            {
                ...
                // 为上一帧时刻的加速度再加上a1 之后除以2就为这段时间的加速度平均值
                // 其中tstep表示a1到上一帧的时间间隔，a0 -
    ...
    void Tracking::Track()
    {
            ...
            !mbCreatedMap)
        {
            ...
            double timePreImu = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(time_EndPreIMU -
                                                                                                      time_StartPreIMU)
                                        .count();
        ...
        else
        {
                ...
                else // 跟踪不正常按照下面处理
                {
                    ...
                    if (mState == RECENTLY_LOST)
                    {
                        ...
                        else
                        {
                            ...
                            // Relocalization
                            bOK = Relocalization();
                            // std::cout << "mCurrentFrame.mTimeStamp:" << to_string(mCurrentFrame.mTimeStamp) <<
                            // std::endl; std::cout << "mTimeStampLost:" << to_string(mTimeStampLost) << std::endl;
            ...
            //          \               \              \---局部地图跟踪失败---false
            //           \               \---当前帧跟踪失败---false
            //            \---上一帧跟踪失败---重定位成功---局部地图跟踪成功---true                       -->OK  2
            //            重定位
            //                          \           \---局部地图跟踪失败---false
            //                           \---重定位失败---false
            ...
            // Save frame if recent relocalization, since they are used for IMU reset (as we are making copy, it shluld
            // be once mCurrFrame is completely modified) 这段貌似没啥作用
            if ((mCurrentFrame.mnId < (mnLastRelocFrameId + mnFramesToResetIMU)) &&
            ...
            if (mState == LOST)
            {
                ...
                // 如果地图中关键帧超过10 并且 纯视觉模式 或
                // 虽然是IMU模式但是已经完成IMU初始化了，保存当前地图，创建新的地图
                CreateMapInAtlas();
    ...
    bool Tracking::NeedNewKeyFrame()
    {
        ...
        if (mSensor != System::MONOCULAR && mSensor != System::IMU_MONOCULAR)
        {
            ...
            // Verbose::PrintMess("[NEEDNEWKF]-> closed points: " + to_string(nTrackedClose) + "; non tracked closed
            // points: " + to_string(nNonTrackedClose), Verbose::VERBOSITY_NORMAL);// Verbose::VERBOSITY_DEBUG);
        }
    ...
    if(nClosedPoints < thStereoClosedPoints && (mSensor==System::STEREO || mSensor==System::IMU_STEREO))
    {
        ...
        // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map
        // matches. Step 7.5：和参考帧相比当前跟踪到的点太少 或者满足bNeedToInsertClose；同时跟踪到的内点还不能太少
        const bool c2 =
    ...
    Sophus::SE3f Tracking::GetCamTwc() { return (mCurrentFrame.GetPose()).inverse(); }
