...
namespace ORB_SLAM3
{
     ...
     * @param sigma 默认为1
     */
    float TwoViewReconstruction::CheckFundamental(const Eigen::Matrix3f &F21, vector<bool> &vbMatchesInliers,
                                                  float sigma)
    {
        const int N = mvMatches12.size();
                                             ...
                                             float minParallax, int minTriangulated)
    {
        ...
        // 4.
        // 看看有没有脱颖而出的结果，且最大值要高于要求的最低值，如果大家都差不多说明有问题，因为4个结果中只有一个会正常
        // 大家都很多的话反而不正常了。。。
        int nsimilar = 0;
     ...
     * @see
     * - Faugeras et al, Motion and structure from motion in a piecewise planar environment. International Journal of
     ...
     * s = det(U) * det(V)  s 有可能是 1 或 -1  ∧ = s^2 * d*U^t * R * V + (U^t * t) * (V^t * n)^t = (s*d) * s * U^t * R
     * * V + (U^t * t) * (V^t * n)^t 令 R' = s * U^t * R * V      t' = U^t * t   n' = V^t * n    d' = s * d ∧ = d' * R'
     * + t' * n'^t    所以∧也可以认为是一个单应矩阵，其中加入s只为说明有符号相反的可能 设∧ = | d1, 0, 0 |    取e1 = (1,
     * 0, 0)^t   e2 = (0, 1, 0)^t   e3 = (0, 0, 1)^t | 0, d2, 0 | | 0, 0, d3 | n' = (a1, 0, 0)^t + (0, b1, 0)^t + (0, 0,
     * c1)^t = a1*e1 + b1*e2 + c1*e3
     *
     ...
     *
     * | x2  x1  0 |     | a1^2 |
     * | 0   x3 x2 |  *  | b1^2 |  =  0    ===>  x1 * x2 * x3 = 0      有(d'^2 - d1^2) * (d'^2 - d2^2) * (d'^2 - d3^2) =
     * 0 | x3  0  x1 |     | c1^2 | 由于d1 >= d2 >= d3  所以d' = d2 or -d2
     *
     * -----------------------------------------------------------------------------------------------------------------------------------------------------------------
                                       ...
                                       vector<bool> &vbGood, float &parallax)
    {
        ...
        for (size_t i = 0, iend = vMatches12.size(); i < iend; i++)
        {
            ...
            // 步骤5：判断3D点是否在两个摄像头前方
            // Check depth in front of first camera (only if enough parallax, as "infinite" points can easily go to
            // negative depth) 步骤5.1：3D点深度为负，在第一个摄像头后方，淘汰
 ...
 * 结论1：如果 S 是实的反对称矩阵，那么S=UBU^T，其中 B 为形如diag(a1​Z，a2​Z...am​Z，0，0...0)的分块对角阵，其中 Z
 = [0, 1; -1, 0]
